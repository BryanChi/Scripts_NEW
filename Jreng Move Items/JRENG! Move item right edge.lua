-- JRENG! (C) MMXV
desc = "Move right edge of selected item(s) to mouse cursor"
r = reaper
function minMax(t)
  local max = -math.huge
  local min = math.huge
...
function move()
  ...
  if string.match(window, "arrange") and segment ~= 'envelope' then
    ...
    dist = (tempo * ((mousePos - maxEdge) / countSel) * .5) / 120

    for i = 0, countSel - 1 do
      reaper.PreventUIRefresh(i + 1)
      item = reaper.GetSelectedMediaItem(0, i)
      reaper.ApplyNudge(0, 0, 0, 15, dist, 0, 0)
    end
  else
    --r.Main_OnCommandEx(42083, 1, 0) -- No pool
